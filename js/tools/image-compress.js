...
(function () {
  ...
  async function handleFiles(files) {
    ...
    for (const file of imageFiles) {
      try {
        ...
        results.push({
          blob, name: newName, originalSize: file.size, ratio,
          img, format, quality,
          width: w, height: h,
  ...
  function appendResultItem(originalName, index) {
    ...
    metaEl.appendChild(document.createTextNode(
      ChoiTool.formatFileSize(r.originalSize) + ' → '
    ));
    ...
    ratioEl.className = 'result-item-ratio ' + (ratioNum > 0 ? 'good' : 'bad');
    ratioEl.textContent = (ratioNum > 0 ? '-' : '+') + Math.abs(ratioNum) + '%';
    ...
    const applyResize = async () => {
      ...
      try {
        ...
        sizeSpan.textContent = ChoiTool.formatFileSize(r.blob.size);
        // 圧縮率更新
        const newRatio = ((1 - r.blob.size / r.originalSize) * 100).toFixed(1);
        const newRatioNum = parseFloat(newRatio);
        ratioEl.className = 'result-item-ratio ' + (newRatioNum > 0 ? 'good' : 'bad');
        ratioEl.textContent = (newRatioNum > 0 ? '-' : '+') + Math.abs(newRatioNum) + '%';
      } catch (e) {
